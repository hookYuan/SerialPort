package com.yuan.serialport.lib;
...
public class SerialPortHelper {
    ...
    private int baud = BaudRate.b115200; //波特率
     ...
     * @param baud     波特率
     */
    public SerialPortHelper(String portPath, int baud) {
        this.baud = baud;
    ...
    public void writeBytes(final byte[] bytes) {
        serialPortWritePool.execute(new Runnable() {
            ...
            public void run() {
                try {
                    if (mSerialPort != null && mSerialPort.getOutputStream() != null && bytes != null && bytes.length > 0) {
    ...
    private boolean isSerialAvailable(@NonNull String portPath) {
        ...
        for (int i = 0; i < mSerialPortFinder.getAllDevicesPath().length; i++) {
            Log.i("TAGQWER", "所有串口------" + mSerialPortFinder.getAllDevicesPath()[i]);
